-- Fix Security Definer View issue by recreating views without elevated privileges
-- The issue is that views owned by postgres can bypass RLS policies

-- Drop and recreate products_with_stats view 
DROP VIEW IF EXISTS public.user_dashboard_stats CASCADE;
DROP VIEW IF EXISTS public.products_with_stats CASCADE;

-- Recreate products_with_stats view
...
-- Recreate user_dashboard_stats view
